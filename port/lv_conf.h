...
/*==============================================================================
 * LVGL 9.4.0 configuration for the NXP i.MX RT1176 (MIMXRT1170-EVKB).
 *
 * DERIVED FROM lvgl/lv_conf_template.h -- everything not listed below is at its
 * upstream template default. On a RE-VENDOR, do not just re-copy the template:
 * re-copy it and then re-apply these, or the port silently loses them.
 *
 * Edited settings (each hunk carries a `RT1176:` comment at its site):
 *   #if 0 -> #if 1               enable the file at all
...
#define LV_STDARG_INCLUDE       <stdarg.h>
    ...
    /* RT1176: 1 MB, sized against the board's 64 MB SDRAM rather than the
     * 256 KB DTCM -- LV_ATTRIBUTE_LARGE_RAM_ARRAY below is what actually puts
     * the pool there. At the template's 64 KB, Task 3/4 render buffers would
     * not fit. */
 ...
 * src/draw/nema_gfx sources need nema_core.h from the deleted lvgl/libs/nema_gfx
 * (unlicensed prebuilt binaries). Enabling this does not fail at config time,
 * it fails at compile time with a missing header. The fix is never to restore
 * the directory. */
...
#if LV_USE_PXP
    /** Use PXP for drawing.*/
    /* RT1176: v1 is software render; PXP acceleration is a later milestone.
     * NOTE this line is INERT -- it sits inside `#if LV_USE_PXP`, which is 0
     * just above. Flipping this alone does NOT enable PXP; LV_USE_PXP must go
     * to 1 as well. Left at 0 so the intent is explicit either way. */
...
/* RT1176: the stock handler is a bare `while(1);`. With LV_USE_LOG 0 that makes
 * any LVGL assertion -- the 1 MB pool exhausting, a NULL from lv_malloc -- an
 * infinite hang with ZERO UART output. Every gate in this project is a UART
 * token grep, so that turns a real failure into a red gate carrying no
 * diagnostic at all. Emit an identifiable token first, THEN halt. */
#define LV_ASSERT_HANDLER_INCLUDE <lvgl_rt1176_assert.h>
#define LV_ASSERT_HANDLER do { lvgl_rt1176_assert_report(); for(;;); } while(0);
 ...
 * RT1176 port: its only use in LVGL is the LV_MEM_SIZE builtin-allocator pool
 * (src/stdlib/builtin/lv_mem_core_builtin.c). At 1 MB that does not fit the
 * 256 KB DTCM, so it goes to the board's 64 MB SDRAM through the core's own
 * EXTMEM section (.externalram -> .bss.extram in imxrt1176.ld). Linker-placed,
 * so no hard-coded SDRAM address and no collision with other EXTMEM data. */
#define LV_ATTRIBUTE_LARGE_RAM_ARRAY __attribute__((section(".externalram"), used))
...
/* RT1176: enabled for heading text (14 is the template default and stays on). */
...
/* MUST STAY 0: source pruned on vendoring (licence) -- see VENDORING.md.
 * src/libs/fsdrv/lv_fs_frogfs.c includes ../frogfs/include/frogfs/frogfs.h,
 * deleted on vendoring (MPL-2.0). Enabling this fails at compile time. */
...
/* RT1176: lvgl/examples was pruned on vendoring (size); nothing to build. */
#define LV_BUILD_EXAMPLES 0

/** Build the demos */
/* RT1176: lvgl/demos was pruned on vendoring (size); nothing to build. */
